#! /usr/bin/env python
# from https://github.com/pycontribs/tendo/blob/master/tendo/singleton.py
...
class SingleInstance:
    ...
    level:
    0, if the python program name is the same, then stop;
    1, if the python program name and path are the same, then stop;
    2, if the python program name, path, and parameters are the same, then stop.

    I think flavor_id is not very useful. --Ben.
    """
    ...
    def __init__(self, flavor_id="", level=2):
        ...
        if level == 0:
            ...
            target = '_'.join([__file__] + sys.argv)
        basename = target.replace("/", "-").replace(":", "").replace("\\", "-") + '-%s' % flavor_id + '.lock'
        #basename = os.path.splitext(os.path.abspath(sys.argv[0]))[0].replace("/", "-").replace(":", "").replace("\\", "-") + '-%s' % flavor_id + '.lock'
        #basename = '_'.join(sys.argv).replace("/", "-").replace(":", "").replace("\\", "-") + '-%s' % flavor_id + '.lock'
