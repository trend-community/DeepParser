#!/usr/bin/python -u
# -*- coding: utf8 -*-
# add system lexicon to use. should be used with the query dict which truncated small frequency queries.
# python2 g1.sent.lexicon.py  /nlpengine.dev/temp/CommentData/000000/Company.txt /nlpengine.dev/temp/CommentData/000000/Company.segmented.3.txt data/g1.10plus.P
...
def LoadDictFromLexicon(dictpath):
    global dict
    with codecs.open(dictpath, encoding="utf-8") as lexicondict:
        for line in lexicondict:
            #print line.strip(), len(line.strip())
            if len(line.strip()) >= 2:
                dict[normalize(line.strip())] = -4
...
print "Before loading from lexicon, size:" + str(len(dict))
LoadDictFromLexicon('../fsa/extra/lexiconlist.txt')
print("After loading from lexicon, size:" + str(len(dict)))
...
for line in fin:
	...
	for chunk in sent.split():
		fout.write( ' '.join( viterbi1(normalize(chunk), dict, maxPhraseLen, args.recursive, args.debug) ) + ' ')
