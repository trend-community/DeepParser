#!/usr/bin/env python
# -*- coding: utf-8 -*-
#source: https://github.com/csdz/nstools/blob/master/zhtools/langconv.py
# use a state machine to do replace (longest principle)
...
if py3k:
    ...
    _zh2Hant, _zh2Hans = {}, {}
    for old, new1 in ((zh2Hant, _zh2Hant), (zh2Hans, _zh2Hans)):
        for k, v in old.items():
            new1[k.decode('utf8')] = v.decode('utf8')
    zh2Hant = _zh2Hant
    zh2Hans = _zh2Hans
    UEMPTY = ''.decode('utf8')
...
class Converter(object):
    def __init__(self, to_encoding, newdict=None):
        global MAPS
        if not MAPS:
            registery('zh-hant', zh2Hant)
            registery('zh-hans', zh2Hans)
            if newdict:
                registery('newdict', newdict)
                logging.info("Loading new dict.")

        self.to_encoding = to_encoding
        if newdict:
            self.map = MAPS['newdict']
        else:
