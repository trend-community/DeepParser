# source: https://phabricator.wikimedia.org/source/mediawiki/browse/master/maintenance/language/zhtable/Makefile.py
...
def main():
    ...
    tbe_dest = 'scim-tables-%s.zip' % SCIM_TABLES_VER
    url = 'https://github.com/scim-im/scim-tables/archive/master.zip'
    download(url, tbe_dest)

    # Get scim-pinyin-$(SCIM_PINYIN_VER).tar.gz:
    url = sfurlbase + 'scim/scim-pinyin-%s.tar.gz' % SCIM_PINYIN_VER
    pyn_dest = 'scim-pinyin-%s.zip' % SCIM_PINYIN_VER
    url = 'https://github.com/scim-im/scim-pinyin/archive/master.zip'
